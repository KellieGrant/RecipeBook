...
export default function RecipeDetail({ recipe, favorite, isOpen, onFavorite, onEdit, onDelete, onBack }) {
  ...
  return <article className={`recipe-detail detail-page ${isOpen ? 'is-open' : ''}`}>
    <div className="detail-actions"><button className="mobile-back" onClick={onBack}>‹ Back</button><button aria-label="Edit recipe" title="Edit recipe" onClick={onEdit}>✎</button><button aria-label="Favorite recipe" title="Favorite recipe" onClick={onFavorite}>{favorite ? '★' : '☆'}</button><button className="delete-recipe" aria-label="Delete recipe" title="Delete recipe" onClick={onDelete}>🗑</button></div>
