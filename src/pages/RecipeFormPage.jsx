...
export default function RecipeFormPage({ recipe, categories, onSave, onCancel }) {
  ...
  const submit = (event) => {
    ...
    onSave({ ...form, image: form.image.trim() || fallbackRecipeImage, image_source: form.image.trim() ? 'manual' : 'fallback', image_attribution: null })
  }
  ...
  return <div className="main-span"><MobilePageHeader title={recipe ? 'Edit recipe' : 'Add recipe'} onBack={onCancel} /><section className="content-page form-page">
    ...
    <form className="recipe-form" onSubmit={submit}>
      ...
      <label className="wide">Image URL <small>Optional</small><input value={form.image} onChange={(event) => update('image', event.target.value)} placeholder="https://…" /></label>
